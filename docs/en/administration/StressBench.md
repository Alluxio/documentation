...
* A Fuse mount point. See [FUSE-based POSIX API](../api/POSIX-API.md) for more details on mounting.
...
* By using Fuse in worker process for reading, which needs to be configured in `${ALLUXIO_HOME}/conf/alluxio-site.properties`
  (see [FUSE-based POSIX API](../api/POSIX-API.md)), the local read
...
Note that Alluxio 2.7 introduced the [register lease](../administration/Scalability-Tuning.md#worker-register-lease)
...
[streaming RPC implementation](../administration/Scalability-Tuning.md#streaming-worker-registration)

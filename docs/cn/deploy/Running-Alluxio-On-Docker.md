...
Alluxio集群中，同时要确保master和worker容器可以在[下面定义的端口互访](../deploy/Requirements.md#基本要求)。
...
单个Alluxio master会导致单点故障。为防止这种情况，生产集群应在[高可用性模式](../deploy/Running-Alluxio-On-a-HA-Cluster.md)下运行多个Alluxio master。
...
你可以在[此处](../deploy/Running-Alluxio-On-a-HA-Cluster.md#选项1--基于raft的嵌入式日志)找到更多Embedded Journal配置信息。
...
您可以在[此处](../deploy/Running-Alluxio-On-a-HA-Cluster.md#选项2--zookeeper和共享日志存储)找到更多关于ZooKeeper和共享日志配置信息。

...
发生这种情况时，需要执行[UFS元数据同步](#ufs元数据同步)操作才能重新使两个名称空间同步。
...
因此，用户可以将不同版本的HDFS挂载到同一个Alluxio命名空间中。 有关更多详细信息，请参考[HDFS底层存储](../ufs/HDFS.md)。
...
有关所支持的Hdfs版本的列表，请参考[HDFS底层存储](../ufs/HDFS.md#支持的hdfs版本)。

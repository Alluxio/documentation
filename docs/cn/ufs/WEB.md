...
首先，本地要有Alluxio二进制包。你可以自己[编译Alluxio](../contributor/Building-Alluxio-From-Source.md)，或者[下载二进制包](../contributor/Building-Alluxio-From-Source.md)

## 配置Alluxio

Alluxio通过[统一命名空间](../core-services/Unified-Namespace.md)统一访问不同存储系统。 WEB的安装位置可以在Alluxio命名空间的根目录或嵌套目录下。
...
[Mount 命令](../operation/User-CLI.md#mount)可以实现这一目的。例如，下面的命令将WEB容器内部的目录挂载到Alluxio的`/web`目录

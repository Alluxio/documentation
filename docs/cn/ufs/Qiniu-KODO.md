...
要在多台机器上运行 Alluxio 集群，需要在这些机器上部署二进制包。你可以自己[由Alluxio源码编译二进制包](../contributor/Building-Alluxio-From-Source.md)，或者[直接下载预编译过的二进制包](../deploy/Running-Alluxio-Locally.md)。
...
Alluxio通过[统一命名空间](../core-services/Unified-Namespace.md)统一访问不同存储系统。 Kodo的挂载点可以在Alluxio命名空间的根目录或嵌套目录下。
...
[Mount 命令](../operation/User-CLI.md#mount)可以实现这一目的。例如，下面的命令将Kodo容器内部的目录挂载到 Alluxio的`/kodo`目录:

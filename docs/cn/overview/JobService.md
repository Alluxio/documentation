...
`fs distributedCp`CLI命令中使用了迁移作业，使用固定的[写入类型](../overview/Architecture.md#data-flow-write)进行数据复制/移动。

### 持久化 Persist

`fs persist` CLI命令间接使用了持久化作业，以`ASYNC_THROUGH`[写入类型](../overview/Architecture.md#data-flow-write)写入Alluxio时使用持久化作业在后台进行持久化。
